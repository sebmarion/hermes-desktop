...
[[src/main/ipc/register.ts#syncSessionCacheForCurrentConnection]] remains the one routing path for local `state.db`, remote HTTP, and SSH/dashboard synchronization. Remote and SSH results are retained in a main-process [[src/main/session-refresh-snapshot.ts#createSessionRefreshSnapshotStore]] keyed by that exact scope; renderer `list-cached-sessions` reads are served from the snapshot and request larger pagination coverage through the same coordinator instead of launching parallel dashboard calls. Each run captures a scope made from connection mode, connection-configuration generation, and active profile. A result for an old scope is discarded, and one newest-scope follow-up starts immediately. Successful current-scope work emits a typed `session-cache-refreshed` generation notice through [[src/preload/session-refresh.ts#subscribeToSessionCacheRefreshed]]. Transient failures retain the currently rendered rows and retry at the next cadence without a toast.

The notice carries no fixed-size row payload. [[src/renderer/src/screens/Layout/SidebarRecentSessions.tsx]] re-reads its exact loaded window plus one sentinel row, preserving infinite-scroll pagination and recomputing `hasMore`; separate notice and pagination generations prevent either race order from truncating or appending stale rows. [[src/renderer/src/screens/Sessions/Sessions.tsx]] quietly re-reads its first 50 cached rows while the modal is visible and only supersedes an initial load after that replacement succeeds. Both consumers compare the notice with the current scope and use request generations so older initial, focus, or background reads cannot overwrite newer results. Their immediate initial loads, focus refreshes, profile/connection changes, mutation refreshes, and pagination remain in place; only the old renderer-owned 60-second and 30-second intervals are removed.
